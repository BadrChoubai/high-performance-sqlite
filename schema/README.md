...
insert into kv
values ('age', 25);
select key, typeof(key), value, typeof(value)
from kv;
...
values ('12345678-1234-1234-1234-123456789123');
select rowid, *
from uuid_example;
...
```sqlite
create table example_id
(
    id integer primary key
);
select rowid, *
from example_id;
insert into example_id
values (1),
       (2);
select rowid, *
from example_id;
...
```sqlite
create table example_no_rowid
(
    id    integer primary key,
    value any
) strict, without rowid;
select rowid
from example_no_rowid;
-- [1] [SQLITE_ERROR] SQL error or missing database (no such column: rowid)
insert into example_no_rowid
values (1, 'apples'),
       (2, 'bananas');
select *
from example_no_rowid;
...
values ('High Performance SQLite', 179.99, 0.0);
select name, price - (price * 0.30) as price_after_discount, price_after_tax
from products;

drop table products;
```

| name                    | price\_after\_discount | price\_after\_tax |
|:------------------------|:-----------------------|:------------------|
| High Performance SQLite | 125.99300000000001     | 179.99            |
